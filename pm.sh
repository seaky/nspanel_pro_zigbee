...
install() {
	...
	if [ ! -d $target_dir ]
	then
		echo "$target_dir does not exist, lets create"
		mkdir -p $target_dir
	fi

	target_dir_data="/data/vendor/siliconlabs_host"
	if [ ! -d $target_dir_data ]
	then
		echo "$target_dir_data does not exist, lets create"
