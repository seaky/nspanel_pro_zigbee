...
class EmberAdapter extends __1.Adapter {
    ...
    async initTrustCenter() {
        ...
        switch (action) {
            case NetworkInitAction.FORM_BACKUP: {
                ...
                await this.formNetwork(true /*from backup*/, backup.networkOptions.networkKey, backup.networkKeyInfo.sequenceNumber, backup.networkKeyInfo.frameCounter, backup.networkOptions.panId, Array.from(backup.networkOptions.extendedPanId), backup.logicalChannel, backup.ezsp.hashed_tclk);
                result = 'restored';
                break;
            }
            case NetworkInitAction.FORM_CONFIG: {
                logger_1.logger.info(`[INIT TC] Forming from config.`, NS);
                await this.formNetwork(false /*from config*/, configNetworkKey, 0, 0, this.networkOptions.panID, this.networkOptions.extendedPanID, this.networkOptions.channelList[0], (0, node_crypto_1.randomBytes)(consts_2.EMBER_ENCRYPTION_KEY_SIZE));
                result = 'reset';
                break;
            }
     ...
     */
    async formNetwork(fromBackup, networkKey, networkKeySequenceNumber, networkKeyFrameCounter, panId, extendedPanId, radioChannel, tcLinkKey) {
        const state = {
    ...
    async addInstallCode(ieeeAddress, key) {
        return await this.queue.execute(async () => {
            ...
            const impStatus = await this.ezsp.ezspImportTransientKey(ieeeAddress, { contents: ZSpec.Utils.aes128MmoHash(key) });
