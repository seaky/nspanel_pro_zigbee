...
function aes128MmoHash(data) {
    const update = (result, data, dataSize) => {
    ...
    for (position; remainingLength >= consts_1.AES_MMO_128_BLOCK_SIZE;) {
        update(hashResult, data.subarray(position, position + consts_1.AES_MMO_128_BLOCK_SIZE), data.length);
    ...
    if (consts_1.AES_MMO_128_BLOCK_SIZE - remainingLength < 3) {
        update(hashResult, temp, consts_1.AES_MMO_128_BLOCK_SIZE);
        temp.fill(0);
    }
...
function checkInstallCode(code, adjust = true) {
    ...
    for (const codeSize of consts_1.INSTALL_CODE_SIZES) {
        ...
        else if (code.length === codeSize - consts_1.INSTALL_CODE_CRC_SIZE) {
            if (adjust) {
